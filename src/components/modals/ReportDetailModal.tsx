...
const getConfiguredSectors = (settings: Array<{ key: string; value: string }>) =>
  (settings.find((item) => item.key === 'form_sectors')?.value || '').split(',').map((sector) => sector.trim()).filter(Boolean);
...
export const ReportDetailModal: React.FC<ReportDetailModalProps> = ({
  report, currentUser, systemSettings, formItems, isEditing, editingData, onClose, onStartEditing, onCancelEditing,
  setEditingData, addEditingReportPhotos, onSaveEdits, onConclude, onApprove, onDelete,
}) => {
  ...
  const visibleItems = formItems.filter((item) => isVisible(item, report.categoria));

  const renderDynamicInput = (field: DynamicFieldDefinition) => {
    const value = editingData.dynamicFieldValues?.[field.id];
    if (field.type === 'textarea') return <textarea value={value || ''} onChange={(e) => setEditingData((c: EditingReportData) => ({ ...c, dynamicFieldValues: { ...c.dynamicFieldValues, [field.id]: e.target.value } }))} className="min-h-[96px] w-full rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-sm text-zinc-300 focus:border-primary focus:outline-none" />;
    if (field.type === 'select') return <select value={value || ''} onChange={(e) => setEditingData((c: EditingReportData) => ({ ...c, dynamicFieldValues: { ...c.dynamicFieldValues, [field.id]: e.target.value } }))} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3 text-sm focus:border-primary focus:outline-none"><option value="">Selecione...</option>{(field.options || []).map((option) => <option key={option} value={option}>{option}</option>)}</select>;
    if (field.type === 'multiselect') { const selectedValues = Array.isArray(value) ? value : []; return <div className="flex flex-wrap gap-2 rounded-xl border border-zinc-800 bg-zinc-950/30 p-3">{(field.options || []).map((option) => { const selected = selectedValues.includes(option); return <button key={option} type="button" onClick={() => setEditingData((c: EditingReportData) => ({ ...c, dynamicFieldValues: { ...c.dynamicFieldValues, [field.id]: selected ? selectedValues.filter((item) => item !== option) : [...selectedValues, option] } }))} className={cn('rounded-lg border px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-all', selected ? 'border-primary/50 bg-primary/15 text-primary' : 'border-zinc-800 bg-zinc-900 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200')}>{option}</button>; })}</div>; }
    if (field.type === 'checkbox') return <label className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900/50 p-4"><input type="checkbox" checked={Boolean(value)} onChange={(e) => setEditingData((c: EditingReportData) => ({ ...c, dynamicFieldValues: { ...c.dynamicFieldValues, [field.id]: e.target.checked } }))} className="h-5 w-5 rounded border-zinc-700 bg-zinc-900 text-primary focus:ring-primary/20" /><span className="text-sm font-bold text-zinc-300">{field.placeholder || field.label}</span></label>;
    return <input type={field.type === 'number' ? 'number' : field.type === 'date' ? 'date' : 'text'} value={value || ''} onChange={(e) => setEditingData((c: EditingReportData) => ({ ...c, dynamicFieldValues: { ...c.dynamicFieldValues, [field.id]: e.target.value } }))} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3 text-sm focus:border-primary focus:outline-none" />;
  };
  ...
  const renderPhotoEditor = () => (
    <div className="space-y-3">
      ...
      {report.photos && report.photos.length > 0 && <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{report.photos.map((photo) => <div key={photo.id} className="space-y-2"><div className="group relative aspect-square overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900"><img src={photo.photo_path} alt={photo.caption || 'Evidência'} className="h-full w-full object-cover" /></div>{photo.caption && <p className="text-[10px] text-zinc-400">{photo.caption}</p>}</div>)}</div>}
    </div>
  );

  const renderField = (item: FormItem) => {
    if (item.id === 'base:photos') {
      return <div key={item.id} className="space-y-2 md:col-span-2"><label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Galeria de evidências</label>{isEditing && report.status === 'Aberto' ? renderPhotoEditor() : report.photos && report.photos.length > 0 ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{report.photos.map((photo) => <div key={photo.id} className="space-y-2"><div className="group relative aspect-square overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900"><img src={photo.photo_path} alt={photo.caption || 'Evidência'} className="h-full w-full object-cover" /></div>{photo.caption && <p className="text-[10px] text-zinc-400">{photo.caption}</p>}</div>)}</div> : <div className="rounded-xl border-2 border-dashed border-zinc-800 p-6 text-center text-zinc-600">Nenhuma fotografia anexada</div>}</div>;
    }
  ...
  return <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 p-0 backdrop-blur-xl no-print md:p-4"><motion.div initial={{ opacity: 0, y: 50, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 50, scale: 0.98 }} className="relative flex h-full w-full max-w-6xl flex-col overflow-hidden border-zinc-800 bg-[linear-gradient(180deg,rgba(17,18,22,0.98),rgba(9,9,11,0.98))] shadow-2xl md:h-[90vh] md:rounded-[2rem] md:border"><div className="flex items-center justify-between border-b border-zinc-800/70 bg-zinc-950/60 px-5 py-5 md:px-7"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><FileText size={20} /></div><div><h3 className="text-xl font-black uppercase tracking-tight">Detalhes da ocorrência</h3><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">ID #{report.id} • {formatDateTime(report.timestamp, 'pt')}</p></div></div><div className="flex items-center gap-2">{report.status === 'Concluído' && <div className="hidden items-center gap-2 rounded-full border border-green-500/20 bg-green-500/10 px-3 py-1 md:flex"><CheckCircle2 size={12} className="text-green-500" /><span className="text-[9px] font-black uppercase tracking-widest text-green-500">Relatório selado</span></div>}<button onClick={onClose} className="text-zinc-500 transition-colors hover:text-white"><XCircle size={24} /></button></div></div><div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 md:max-h-[80vh]"><div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="grid grid-cols-1 gap-4 md:grid-cols-2">{visibleItems.map(renderField)}</div><div className="space-y-4"><div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Agente responsável</p><div className="mt-3 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-sm font-black text-black">{report.agente_nome?.charAt(0) || '?'}</div><div><p className="text-xs font-black uppercase text-zinc-200">{report.agente_nome}</p><p className="text-[10px] uppercase tracking-widest text-zinc-500">{report.agente_nivel}</p></div></div></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4"><div className="flex items-center gap-2 text-zinc-500"><MapPin size={14} /><span className="text-[10px] uppercase tracking-widest">LAT: {report.coords_lat ? Number(report.coords_lat).toFixed(4) : 'N/A'}</span></div><div className="mt-2 flex items-center gap-2 text-zinc-500"><MapPin size={14} /><span className="text-[10px] uppercase tracking-widest">LNG: {report.coords_lng ? Number(report.coords_lng).toFixed(4) : 'N/A'}</span></div></div></div></div></div><div className="no-print border-t border-zinc-800 bg-zinc-900/40 p-4 md:p-6"><div className="flex flex-col gap-4"><div className="flex flex-col items-center justify-between gap-4 rounded-xl border border-zinc-800/50 bg-zinc-900/50 p-3 sm:flex-row"><p className="text-center text-[9px] font-bold uppercase tracking-widest text-zinc-600 sm:text-left md:text-[10px]">Sistema de Segurança MineGuard • Auditoria ativa</p><div className="flex items-center gap-2"><button onClick={() => window.print()} className="rounded-lg border border-zinc-700 bg-zinc-800 p-2.5 text-zinc-300 transition-all hover:bg-zinc-700" title="Exportar PDF"><Printer size={16} /></button><button onClick={onClose} className="rounded-lg bg-zinc-100 px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-black transition-all hover:bg-white">Fechar</button></div></div><div className="flex flex-wrap justify-center gap-2 sm:justify-end">{isEditing && report.status === 'Aberto' ? <><button type="button" onClick={onCancelEditing} className="rounded-lg border border-zinc-700 bg-zinc-800 px-8 py-2.5 text-[10px] font-black uppercase tracking-widest text-zinc-300 transition-all hover:bg-zinc-700">Cancelar</button><button type="button" onClick={onSaveEdits} className="rounded-lg bg-blue-600 px-8 py-2.5 text-[10px] font-black uppercase tracking-widest text-white transition-all hover:bg-blue-500">Guardar alterações</button></> : <div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-2 lg:flex lg:flex-wrap">{report.status === 'Aberto' && <button type="button" onClick={onStartEditing} className="flex items-center justify-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-black transition-all hover:bg-primary/90">Editar</button>}{report.status !== 'Aprovado' && (report.status !== 'Concluído' || currentUser?.permissions?.conclude_reports) && <button onClick={() => onConclude(report.id, report.status || 'Aberto')} className={cn('flex min-w-[120px] items-center justify-center gap-2 rounded-lg px-6 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all', report.status === 'Concluído' ? 'border border-zinc-700 bg-zinc-800 text-zinc-400 hover:bg-zinc-700' : 'bg-green-600 text-white hover:bg-green-500')}>{report.status === 'Concluído' ? <><Clock size={14} />Reabrir</> : <><CheckCircle2 size={14} />Concluir</>}</button>}{report.status !== 'Aprovado' && report.status === 'Concluído' && currentUser?.permissions?.approve_reports && <button onClick={() => onApprove(report.id)} className="flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-white hover:bg-indigo-500"><Shield size={14} />Aprovar</button>}{(String(currentUser?.nivel_hierarquico).toLowerCase() === 'superadmin' || String(currentUser?.nivel_hierarquico).toLowerCase() === 'admin' || currentUser?.permissions?.delete_reports || currentUser?.id === 1) && <button onClick={() => onDelete(report.id)} className="flex items-center justify-center gap-2 rounded-lg border border-red-500/20 bg-red-600/10 px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-red-500 transition-all hover:bg-red-600 hover:text-white"><Trash2 size={14} />Eliminar</button>}</div>}</div></div></div></motion.div></div>;
};
